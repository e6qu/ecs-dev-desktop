...
#   - attaches the least Amazon ECR policy needed by the release workflows
...
echo "edd: attaching release role Amazon ECR push policy"
...
if aws iam get-role-policy \
  --role-name "$role_name" \
  --policy-name "EddReleasePublishDeploy" >/dev/null 2>&1; then
  echo "edd: deleting legacy release role publish/deploy policy"
  aws iam delete-role-policy \
    --role-name "$role_name" \
    --policy-name "EddReleasePublishDeploy" >/dev/null
fi
