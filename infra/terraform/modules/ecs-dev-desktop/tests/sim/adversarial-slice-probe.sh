...
# Bound the window to events generated by this probe so prior test runs do not
# cause an unbounded (or endless) pagination loop.
...
while [ "$pages" -lt "$max_pages" ]; do
  ...
  if [ -z "$next_token" ]; then
    out=$(aws cloudtrail lookup-events --start-time "$window_start" --end-time "$window_end" --max-results 10 --output json)
  else
...
pass "CloudTrail LookupEvents pagination returned $total events across $pages pages"
...
aws kms delete-alias --alias-name "$key_alias" >/dev/null 2>&1 || true
aws ecs delete-cluster --cluster edd-ct-timegate >/dev/null 2>&1 || true
