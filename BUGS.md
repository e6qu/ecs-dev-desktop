...
- **CloudWatch alarm SNS notification delivered to SQS has malformed JSON body and is flaky — OPEN (2026-07-01).** Originally filed as **e6qu/sockerless#734**. sockerless #739 fixed the JSON body format when direct SNS → SQS delivery occurs, sockerless #741/#742 addressed process-mode SNS → SQS fan-out, but **e6qu/sockerless#745** tracks that CloudWatch alarms still do not trigger an SNS `Publish` when transitioning to `ALARM` in `SIM_RUNTIME=process`. The probe now fails loudly on missing SQS delivery; CI stays red until #745 is fixed and verified.
