...
- **CloudWatch alarm SNS notification on ALARM transition — STILL OPEN after sockerless #751 (2026-07-02).** Originally filed as **e6qu/sockerless#734**. sockerless #739 fixed the JSON body format when direct SNS → SQS delivery occurs, sockerless #741/#742 addressed process-mode SNS → SQS fan-out, and **sockerless #748** added an isolated CLI regression test that passes in a fresh simulator subprocess. The integrated `terraform-sim` environment still failed after Terraform apply/destroy cycles; filed **e6qu/sockerless#749**. **sockerless #751** attempted to fix #749 by resetting the CloudWatch alarm evaluator state on `PutMetricAlarm` and the submodule was re-pinned to `3d85b89`, but the integrated probe still fails with no `SNS.Publish` logged. Filed **e6qu/sockerless#753** as a follow-up.

- **bleephub `GET /user/teams` returns 403 Forbidden — OPEN (2026-07-02).** After bumping `third_party/sockerless` from `38e311ac` (#737) to `3d85b89` (#751), the bleephub sim returns 403 Forbidden for `GET /user/teams`, which breaks Auth.js GitHub provider team mapping. Both the `e2e` and `e2e-https` CI jobs fail with `Error: GitHub /user/teams failed: 403 Forbidden`. Filed **e6qu/sockerless#754**.
