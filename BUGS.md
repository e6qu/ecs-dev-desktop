...
- **Shauth catalog entry crashed, relying-party logout immediately signed the user back in, and the provider-session index broke every non-Shauth callback — fixed 2026-07-19.** The catalog route called Auth.js from a React Server Component, where callback/state/PKCE/nonce cookie mutation was forbidden, and application logout cleared only the local Auth.js cookie before returning to a still-active Shauth browser session. The launch became a cookie-capable Next.js Route Handler. Shauth sign-in persisted the verified provider `sid` and ID token in the durable session ledger; application sign-out used Shauth's standard RP-Initiated Logout endpoint; and a signed OIDC Back-Channel Logout endpoint revoked all local sessions correlated with the provider `sid`. The post-logout redirect became the registered EDD-origin `/signed-out` page instead of the invalid Shauth portal origin, while non-Shauth sign-out returned to the ordinary login page and did not claim that a global Shauth session had ended. Every durable session also received complete provider-session index facets, so GitHub, Microsoft Entra ID, and local-account callbacks no longer failed ElectroDB validation. Invalid issuer, audience, age, event, `sid`, `jti`, signature, and nonce shapes failed closed.
