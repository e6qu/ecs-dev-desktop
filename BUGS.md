...
- **CloudWatch alarm SNS notification on ALARM transition — STILL OPEN after sockerless #756/#759 (2026-07-03).** Originally filed as **e6qu/sockerless#734**. sockerless #739 fixed the JSON body format when direct SNS → SQS delivery occurs, sockerless #741/#742 addressed process-mode SNS → SQS fan-out, **sockerless #748** added an isolated CLI regression test that passes in a fresh simulator subprocess, **sockerless #756** moved the evaluator's last-dispatched state onto each alarm's persisted state, and **sockerless #759** added a dangling-alarm regression test. The integrated `terraform-sim` probe still fails with no `SNS.Publish` logged. Filed **e6qu/sockerless#760** as a follow-up.
