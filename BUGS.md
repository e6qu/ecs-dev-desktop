...
- **CloudWatch alarm SNS notification on ALARM transition — FIXED upstream by sockerless #751 (2026-07-02).** Originally filed as **e6qu/sockerless#734**. sockerless #739 fixed the JSON body format when direct SNS → SQS delivery occurs, sockerless #741/#742 addressed process-mode SNS → SQS fan-out, and **sockerless #748** added an isolated CLI regression test that passes in a fresh simulator subprocess. The integrated `terraform-sim` environment still failed after Terraform apply/destroy cycles; filed **e6qu/sockerless#749**. **sockerless #751** fixed #749 by resetting the CloudWatch alarm evaluator state on `PutMetricAlarm`, so re-created alarms dispatch `AlarmActions` on the first real `ALARM` transition. The submodule is re-pinned to `3d85b89`; PR #180 is now being verified in CI.
