...
- **CloudWatch alarm SNS notification delivered to SQS has malformed JSON body and is flaky — OPEN (2026-06-30).** Filed as **e6qu/sockerless#734**. The alarm action fires and the alarm transitions to `ALARM`, but the SNS → SQS message body is intermittently malformed (unescaped quotes) and sometimes not delivered at all in process mode. The wave-3 CloudWatch Alarm → SNS adversarial probe (`adversarial-slice-cloudwatch-alarm-sns.sh`) verifies alarm creation, state transition, and AlarmActions wiring, but skips the SQS receipt assertion pending the upstream fix.
