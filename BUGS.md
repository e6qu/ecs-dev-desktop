...
- **CloudWatch alarm SNS notification on ALARM transition — PARTIALLY FIXED upstream by sockerless #764; new regression filed as e6qu/sockerless#766 (2026-07-03).** Originally filed as **e6qu/sockerless#734**. sockerless #739 fixed the JSON body format when direct SNS → SQS delivery occurs, sockerless #741/#742 addressed process-mode SNS → SQS fan-out, **sockerless #748** added an isolated CLI regression test, **sockerless #756** moved the evaluator's last-dispatched state onto each alarm's persisted state, **sockerless #759** added a dangling-alarm regression test, **sockerless #761** fixed the race by moving state read/dispatch/write into a single `cwAlarms.Update` callback, and **sockerless #764** added fan-out observability logging at every SNS→SQS decision point. In the integrated `terraform-sim` environment the fan-out now logs `SNS to SQS delivery succeeded`, but polling the subscribed SQS queue for 30 seconds returns an empty `Messages` array. Filed **e6qu/sockerless#766**. The submodule is re-pinned to `6756ecfb`; PR #180 is blocked on the upstream fix.

- **bleephub `GET /user/teams` returns empty list — NOT FIXED; filed e6qu/sockerless#765 (2026-07-03).** sockerless #756 removed the OAuth-scope gate, fixing the 403 Forbidden regression (**e6qu/sockerless#754**), but the endpoint returned an empty team list. **sockerless #764** claimed to fix this by emitting `X-OAuth-Scopes` for web-flow tokens. After re-pinning to `6756ecfb`, our GitHub OAuth e2e still sees `GET /user/teams` return `[]` for the OAuth web-flow token, so Auth.js maps the test user to `viewer` instead of `admin`. Filed **e6qu/sockerless#765** as a follow-up to **e6qu/sockerless#763**.
