...
- **CloudWatch alarm SNS notification on ALARM transition — FIXED upstream by sockerless #756 (2026-07-03).** Originally filed as **e6qu/sockerless#734**. sockerless #739 fixed the JSON body format when direct SNS → SQS delivery occurs, sockerless #741/#742 addressed process-mode SNS → SQS fan-out, **sockerless #748** added an isolated CLI regression test that passes in a fresh simulator subprocess, and **sockerless #756** moved the evaluator's last-dispatched state onto each alarm's persisted state (so replacement naturally resets it) and added evaluator-loop panic recovery. The integrated `terraform-sim` probe is now expected to pass. The submodule is re-pinned to `a3448639`; PR #180 is running through CI for verification.
