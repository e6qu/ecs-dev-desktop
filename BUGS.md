...
- **CloudWatch alarm SNS notification on ALARM transition — FIXED upstream by sockerless #761 (2026-07-03).** Originally filed as **e6qu/sockerless#734**. sockerless #739 fixed the JSON body format when direct SNS → SQS delivery occurs, sockerless #741/#742 addressed process-mode SNS → SQS fan-out, **sockerless #748** added an isolated CLI regression test, **sockerless #756** moved the evaluator's last-dispatched state onto each alarm's persisted state, **sockerless #759** added a dangling-alarm regression test, and **sockerless #761** fixed the race by moving state read/dispatch/write into a single `cwAlarms.Update` callback. The integrated `terraform-sim` probe is now expected to pass. The submodule is re-pinned to `354c81d3`; PR #180 is running through CI for verification.

- **bleephub `GET /user/teams` returns 403 Forbidden — FIXED upstream by sockerless #756 (2026-07-03).** The endpoint no longer requires an OAuth scope, matching real GitHub behaviour. Filed as **e6qu/sockerless#754**.
