...
- **CloudWatch alarm SNS notification delivered to SQS has malformed JSON body and is flaky — OPEN (2026-07-01).** Filed as **e6qu/sockerless#734**. sockerless #739 fixed the JSON body format when delivery occurs, but **e6qu/sockerless#741** tracks that the delivery itself still does not occur in `SIM_RUNTIME=process`: the alarm transitions to `ALARM`, but no message reaches the subscribed SQS queue. The wave-3 CloudWatch Alarm → SNS adversarial probe (`adversarial-slice-cloudwatch-alarm-sns.sh`) now fails loudly on missing SQS delivery, so CI will stay red until #741 is fixed and verified.
