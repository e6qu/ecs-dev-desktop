...
- ~~**The control-plane and SSH gateway services had no health check grace period, so a starting task was replaced before it could answer its load balancer (Scaleway deploy of sockerless 0.32.7, 2026-09-15). FIXED 2026-09-16.**~~ After the simulator restart the control plane took 25-26 s from task start to listening. The target group marks a target unhealthy after three failed `/api/readyz` checks ten seconds apart, and with no `health_check_grace_period_seconds` the service scheduler acts on that verdict at once: task `741fe67e` was stopped for "Task failed ELB health checks" 51 s after starting and 25 s after it began listening, its replacement `d6dbdd67` failed the same way while booting, the scheduler launched a third task, and trimmed it as surplus once `d6dbdd67` passed. The simulator's health checker behaves as Elastic Load Balancing does here (a new target is `initial` until a check succeeds or the unhealthy threshold fails), so the gap was the module's: AWS documents the grace period for exactly this. The control plane now has 120 s and the SSH gateway, whose TCP check needs only two failures, 60 s; `existing-environment.tftest.hcl` asserts both. That test had not been running: CI never invoked `terraform test`, and since #263 made `aws_endpoint_url` required every run failed on the missing variable. The test file now sets it, and the `terraform` job runs the module's tests.

- ~~**Golden-image validation could not fit its 15-minute budget on a branch's first run, and no image build ever reused another commit's cache (ecs-dev-desktop #288, 2026-09-15). FIXED 2026-09-15.**~~ The pull-request job built the base and all five language variants in sequence: the base took 4.3 minutes cold, and the variants were plain `docker build`s on a runner with an empty layer store, so they never had a cache at all (Python 1.5, Go 2, Java 2.8 minutes, Rust past 3.7 still compiling cargo-audit). The base's GHA cache helped only after a first run on the same branch, because main never runs the validation job and so never saved that scope. The run was cancelled at 15 minutes 27 seconds. Behind it: `scripts/publish-images.sh` named every GHA cache scope after the full image reference, commit tag included, so each main-branch publish (golden base, variants, control plane, SSH gateway) wrote a cache no later commit read. Fix: scopes name the image and architecture, not the tag; validation is a base job, which pushes a run-scoped `edd-ci-golden-base` fixture, and one job per variant, which builds FROM it with buildx under main's variant cache scope and smoke-tests only that variant — each unit under its own 15-minute cap, as the publish jobs already were.

- ~~**The control plane built its AWS clients on every request, so the monitoring observation Shauth polls with a five-second budget kept missing it (6,694 ms on the 2026-09-15 post-apply gate after #287 had parallelised the health checks). FIXED 2026-09-15.**~~ `getHealthService()` and `getInfrastructureService()` constructed a DynamoDB client and, through `activeProviders()`, an EC2 and an ECS client on every call; `checkReadiness()` built a DynamoDB client per ALB readiness probe; `getAuditSource()`, `getMetricReader()` and `getLogSource()` rebuilt their CloudTrail, CloudWatch and DynamoDB clients per request. A new SDK client resolves the task role's credentials again and has no warm connections, so every one of those requests paid that before doing any work. They are built once per process now, like every other service in `control-plane.ts`; a unit test calls each getter three times and requires each factory to have run once. The larger share of the observation's time was the Sockerless simulator's DynamoDB, whose index queries read every item of the table under its lock (fixed in sockerless-cloud).

- ~~**Shauth's monitoring abandoned `GET /api/observations` and failed the post-apply gate on 88286d7af9cd (2026-09-14): the health report awaited its dependency checks one after another. FIXED 2026-09-14.**~~ Shauth gives up on an application's observation after five seconds; the control plane served it in 5,777 ms, and every recorded collection since 2026-09-11 had taken 3.3–8.5 s (earlier gates failed before reaching the monitoring card). `HealthService.report()` awaited the DynamoDB ping, the compute and storage checks, the reconciler heartbeat and the GitHub App installations call in sequence, so their round-trips added up. They run concurrently now, with the components in the same order; a unit test requires every check to have started before the report's promise is returned, which the sequential version fails without any timing. The DynamoDB ping is a `DescribeTable`, which the Sockerless simulator had made read every item to answer (sockerless-cloud BUG-3002, fixed there).

- ~~**The admin workspace list took 32 s and failed the post-apply gate on 1e56f50a80eb (2026-09-14): every whole-fleet read scanned the single table, and session rows never expired. FIXED 2026-09-14.**~~ The gate signs in as the bootstrap admin and waits 30 s for `GET /api/workspaces`; the control plane logged `workspaces.list` at 32,316 ms. The authenticated list had grown from 2.4 s on 2026-09-11 to 28.8 s that morning, while the table held 7,432 items for 4 workspaces: 3,876 session correlations, 1,938 sessions, 1,056 logout tokens, 431 audit events. Two defects:
  1. **`list()` for an admin, the reconciler's keep-sets (`listReferencedStorage`, `listFleetReferences`, `listReferencedTasks`, `listRuntimeSecretWorkspaceIds`), `reconcileOwnerCounts` and the derived audit feed each ran `workspaces.scan`.** In a single-table design that reads every entity to find the workspaces, so each got slower with every sign-in, and the reconciler paid it every maintenance tick. They now query the state index (GSI2) once per state; `WORKSPACE_STATES` names every state and fails to compile when one is added to `WorkspaceState` without it. An integration test seeds a workspace in each state beside 200 unrelated sessions and records the DynamoDB commands: no Scan.
  2. **Session rows carried no time-to-live attribute.** The table's TTL is `expiresAtEpochSeconds`; correlations and logout tokens set it, but `authSession` stored only the ISO `expiresAt`, which DynamoDB's TTL ignores, so every session ever created stayed. Sessions now write `expiresAtEpochSeconds` on create and on each rolling refresh. Rows written before this carry none and are not expired by TTL.

  3. **Harness clients ignored `AWS_ENDPOINT_URL`.** Running the web integration suite against a private simulator for this fix, `observability-live.integ.ts` connected to 127.0.0.1:4566 (refused, nothing was listening): it set `AWS_ENDPOINT_URL` and then built its ECS, CloudWatch Logs and CloudTrail clients from `aws.endpoint`, the harness constant. so did `cloudtrail-audit-source.integ.ts`, `awsSimClientConfig()` in `packages/e2e` did the same for every e2e suite, `live-ecs-app.ts` and `user-journey.e2e.ts` passed the constant to the app they start, and `live-cloud-setup.ts` exported the override and then the constant after it, so the constant won. They all use the run's endpoint now, the constant only as its default.

  Also found: the Sockerless simulator stored a table's TTL setting but never deleted an expired item, which is why the expired correlations and logout tokens were still there (fixed in sockerless-cloud). Still a scan: `reconcileOwnerCounts` reads the owner counters with a scan (the counter entity has no index to enumerate it), and the admin console's session and invitation lists scan; with TTL deleting expired rows the table they read is bounded by live sessions.

- ~~**A freshly launched workspace never read as usable in time, and the fault ran through the heartbeat and launch path — the 2026-09-14 post-apply gate on 6d0496e3f369. FIXED 2026-09-14.**~~ The gate's terminal workspace sat `running` with no functional report until its 8-minute deadline. Its idle-agent logged `heartbeat failed`; the control plane had answered 409 to both beats — the first because the workspace was still `provisioning` (the launch took 6.5 minutes, and the agent is up and reporting well before the launch's readiness poll commits `running`), the second because the gate had already deleted it. Reproduced against a private simulator at the deployed release. The defects:
  1. **An agent report during provisioning was refused**, and the next came a whole heartbeat interval later, so readiness after every launch waited on a beat that happened to land after `running`. The reports are facts about the desktop, not transitions: `heartbeat()` now records the functional and session reports while `provisioning`, without recording activity (provisioning has no idle window).
  2. **`launchReserved` committed `running` against the version it read before a launch that can take minutes**, so any write to the record in between — routine once (1) is fixed — lost the commit and stopped the healthy task. It re-reads the record after RunTask, as the wake path already did; a record that moved elsewhere is still a conflict and its task is still stopped.
  3. **A session with nothing to resume turned every heartbeat into a 500.** `edd-shell` records every OpenVSCode terminal as a `shell` session whose `resumeCommand` is null; the contract allows null, ElectroDB's `string` attribute does not, so from the first opened terminal on every beat threw — the functional report and activity stopped updating, and the reconciler could scale an in-use workspace to zero. Stored without the attribute, read back as null.
  4. **Sessions were written but never read back**: `toWorkspace` had no mapping for `sessions`/`sessionsAt`, so the status page's session list (#284) was always empty.
  5. **The functional report outlived a stop** (`markStopped` kept it), so a woken workspace read "ok" before its container existed. Cleared on stop.
  6. **The heartbeat interval equalled the idle threshold.** `@edd/core`'s `DEFAULT_HEARTBEAT_INTERVAL_S` and the module's `heartbeat_interval_s` were 300 s against a 300 s idle threshold measured from the last beat, and a beat takes time of its own, so a workspace kept alive only by its agent (SSH, background compute) could age past the threshold between beats and be scaled to zero while in use. Both are 120 s now — the idle-agent's own default — the unused `@edd/config` duplicate (whose comment still cited a 30-minute threshold) is gone, and the module refuses a plan whose interval does not fit twice inside the idle threshold.

  Integration tests cover a heartbeat landing mid-launch, a session with no resume command round-tripping through repeated heartbeats, and stop clearing the report; a core test covers `markStopped`.

- ~~**CI's `terraform init` downloaded providers from releases.hashicorp.com on every run and had no way to survive a dropped download (two `terraform-sim` slices failed on 2026-09-13 with `read: connection reset by peer` fetching cloudinit 2.4.1, on a change that touched no Terraform). FIXED 2026-09-14.**~~ The slice job cached `tests/sim/.terraform/providers` per directory, so the `terraform` and `check-deps` jobs never had a cache and the slices' own missed whenever the key moved; and Terraform's installer has no retry, so one TCP reset failed the job. The deeper cause: the CI workflow runs only on pull requests, and GitHub lets a pull request restore caches only from its own branch and its base branch — nothing ever ran on `main`, so nothing ever saved a cache there, and every new branch started cold and downloaded every provider. Fix: `.github/workflows/warm-caches.yml` warms the plugin cache on `main` (on lock-file changes, weekly, and on demand) under the exact key CI restores; one workflow-wide `TF_PLUGIN_CACHE_DIR` restored by `actions/cache` in every terraform job (keyed on every lock file, `restore-keys` so a bump fetches only the moved provider); and `scripts/terraform-init.sh`, which re-runs `init` at most three times ONLY when the failure is a broken provider download — a constraint error or a bad configuration fails at once as before. Every `terraform init` in CI and in `check-latest-deps.sh` goes through it.

- ~~**Every non-OpenVSCode workspace reported "degraded — IDE unreachable" after the editor manager landed; the Scaleway post-apply gate caught it on the first deployment of 85b2b028be73 (2026-09-13). FIXED 2026-09-14.**~~ #281 moved the idle-agent's IDE probe from the editor's public port to the editor manager's status port (:3002) so a heartbeat would never wake an unloaded editor — but only `openvscode` mode runs the manager; `monaco`, `terminal` and `opencode` serve :3000 themselves and have nothing on :3002, so their probe failed every beat and the control plane recorded `functional: degraded, "IDE unreachable on :3000"` (a message naming a port the probe was not even asking) while the desktop served fine. Not caught in CI because the e2e fixtures only waited for `state: running`, not for the agent's `functional: ok` — the readiness the status page and the deployment gate actually judge by. Fix: the probe is mode-aware (`EDD_EDITOR_MODE`): the manager's status port where a manager fronts the editor, the public port where the mode serves it directly and nothing unloads; `edd-idle-agent --probe` prints one self-report, and `infra/images/base/smoke.sh` asserts `"ide":true` from inside the container for each of the four modes; `createRunningWorkspace` in the e2e fixture now waits for `functional: ok` (with the detail in the failure message) so every terminal/monaco e2e flow fails on a probe that cannot see its desktop; the core message no longer names a port.

- ~~**A RunTask placement refusal ends the launch in `error`, where real Fargate expects the caller to try again (found 2026-09-13, while making the simulator refuse placement — sockerless-cloud#162). FIXED 2026-09-13.**~~ `EcsComputeProvider.runTask` read `failures[0].reason`/`.detail` correctly but threw a generic Error, and `WorkspaceService` recorded every launch throw with `recordLaunchFailure`, so a workspace Fargate could not place *right now* ("Capacity is unavailable at this time. Please try again later or in a different availability zone" — documented by AWS as transient) landed in `error` with a Retry button, indistinguishable from a broken task definition; the path had never run because the simulator never refused placement before sockerless-cloud#162. Fix: the compute port has a typed `PlacementRefusedError`; on it the record stays `provisioning`, waiting for capacity (`deferPlacement` sets `placementReason` / `placementAttempts` / `placementRetryAt`, 15 s → 30 s → 60 s → 2 min, and restarts the stuck-provisioning clock), the lifecycle converge sweep re-runs due launches (`listPlacementDue` + `retryPlacement`; a wake re-hydrates from its snapshot, never a fresh volume), and only after `MAX_PLACEMENT_ATTEMPTS` (8, ~12 min) does the launch fail the ordinary way — a create to `error` with Retry, a wake back to `stopped`. `connect()` treats a waiting wake as a wake in flight. The status page shows the wait, the attempt and the reason; `workspace.placement.refused` counts refusals by reason. Covered by core policy tests and control-plane integration tests (create and wake, the due gate, the spent budget on both paths) against `FakeComputeProvider.refuseNextPlacements`.
...
- **Private repositories could not clone in the dev environment, and the launcher offered a "Connect GitHub" button that returned a 500 — fixed 2026-09-07.** The dev control plane had `EDD_TOKEN_ENC_KEY` but neither `EDD_GITHUB_APP_ID`/`EDD_GITHUB_APP_KEY` nor `AUTH_GITHUB_ID`/`AUTH_GITHUB_SECRET`; with Shauth-only sign-in no session ever captured a token, so every private clone failed at boot with the credential broker's old `404`. Nothing told an admin. The git-access state is now a pure function of configuration (`lib/git-integration.ts`): the launcher hides account linking when it is not configured and explains the alternatives, `/api/github/connect/start` answers `409` with the reason, the Health board carries a `git-integration` row (degraded when nothing is configured; a GitHub App is exercised live), and `docs/deploying.md` documents the three paths. The deployment-side fix (installing a GitHub App on `e6qu`) belongs to the private infra repo.

- **A just-added base image did not appear in the session launcher for up to 10 s — fixed 2026-09-07.** `getCatalogList` is TTL-cached with no invalidation, so a catalog write in the same process stayed invisible to the launcher, workspaces list, and API until the TTL lapsed (it made the metadata-picker portal test fail once the suite grew). Every catalog mutation route and the image-tag rollout now invalidate the cache; other replicas still converge within the TTL.

- **The Playwright portal spec seeded a duplicate catalog entry on every worker restart — fixed 2026-09-07.** Entries are keyed by id, so each restart added another "Node 20" for the same image and the picker showed several selected. The seed is now idempotent by image.

- **A session from a nonexistent repository URL was created, booted, and only then failed its clone with git's opaque "could not read Username" — fixed 2026-09-07.** A user created a workspace from `https://github.com/e6qu/pos3q.git` (the public repository is `e6qu/pos3ql`); GitHub answers `401` for a repository that does not exist exactly as for a private one, so git asked the in-workspace credential helper, the control-plane broker answered `404 no credential` (the owner had no linked account), and the boot log showed `could not fetch a git credential … 404` followed by `fatal: could not read Username for 'https://github.com': terminal prompts disabled`. The launcher's public-URL mode validated only the URL's shape. Three repairs: (1) `POST /api/workspaces` now checks the repository BEFORE reserving the session, the way `git clone` starts — a smart-HTTP ref advertisement (`GET <repo>/info/refs?service=git-upload-pack`) with the same credential the workspace will present (none for a public repository) — and refuses with a `422` that names the URL and the fix (`… was not found, or it is private. Check the URL, or connect your Git account …`), also catching a branch or tag the repository does not have and an unreachable host; the launcher shows that message and stays on the form. (2) The broker answers an empty `204` (not `404`) when the owner has no credential, so the helper distinguishes "no linked account" from "workspace gone" and says so. (3) The bootstrap status file explains that hosts answer "authentication required" for a missing repository too. Unit tests cover the pkt-line parser (property-based), ref matching, the message table, and the probe; the create-route integration test runs a TLS git-host fixture (throwaway certificate trusted per-process via `tls.setDefaultCACertificates`); a portal browser test proves the refusal reaches the launcher with nothing created. The public-URL mode was generalised from `github.com` only (a hardcoded endpoint) to any HTTPS git host, since the check works for all of them.

- **Light mode was unreadable: the top bar stayed dark under dark text (1.6:1), and the green accent carried white text at 3.3:1 — fixed 2026-09-07.** The light-theme `.topbar` override sat inside the `@media (prefers-color-scheme: light)` block ABOVE the base `.topbar { background: rgba(9,11,10,.78) }` rule, so the later rule of equal specificity won and light mode rendered dark chrome with `#121b13` text on it; `--accent: #4f9f00` was used as text and as a button surface under white ink at 3.3:1. The previous "WCAG contrast" coverage measured one link on one page. Repairs: the top-bar background, focus halo, chip/code surfaces, and error-notice text became theme tokens (`--chrome`, `--focus-halo`, `--chip-bg`, `--code-bg`, `--notice-text`) instead of dark literals; the light accent became `#367000` (6.0:1 under white, 4.5:1+ as text on every light surface) with `--st-running` to match; stale `var(--x, #literal)` fallbacks in components (which disagreed with the tokens) were dropped. A new `e2e/a11y.pw.ts` audits every portal route (30 routes, anonymous/developer/admin) with axe-core against WCAG 2.2 AA in BOTH colour schemes, waiting for finite animations to settle first (axe sampled the card fade-in mid-flight and read a blended background); 60 audits pass.

- **OpenVSCode logged two `File not found: …/vsda…` errors into every workspace's boot log on each editor open — fixed 2026-09-07.** The OSS build ships no `vsda` (Microsoft's proprietary connection-signing module) yet the web workbench fetches `vsda.js` and `vsda_bg.wasm` on every connect and proceeds unsigned on a 404; the server logs each miss to stderr → CloudWatch → the portal's boot log. The in-app editor proxy now answers those two OpenVSCode paths with 404 itself (pure `isAbsentEditorAsset` gate, unit-tested), so the browser outcome is identical and the workspace log stays truthful.

- **The metadata-picker portal test asserted which catalog entry the launcher preselects — fixed 2026-09-07.** The picker preselects the first entry the catalog lists, an order that depends on generated ids; the test only passed while its seed was the sole entry. It now selects its option and asserts that.

- **typescript-eslint 8.69 flagged three `void x;` no-ops — fixed 2026-09-07.** `WorkspaceService.requestStop` discarded an `actor` it already used; `SnapshotsConsole` used `void nonce` to force a `useCallback` identity change (usePoll gained an explicit `refreshKey` instead); `decideImageSourceBuild` took a `paths` argument it ignored (parameter removed).

- **The workspace fixture exceeded its 15-minute ceiling and copy-pasted retention failed on a transient GitHub API response — fixed 2026-07-21.** The original job built the shared base, built the omnibus workspace, exported it locally, and only then began a large serial upload. A measured run completed both builds but was still uploading when the job was cancelled. The base moved to an exact run-scoped package in its own bounded job, the established `e2e fixture (workspace)` check consumed that immutable coordinate through direct BuildKit publication, and the simulator published independently. The measured split completed the base in 5m17s and the workspace in 12m12s. All three packages used one POSIX-shell retention helper that retained exactly 20 versions, retried only transient idempotent GitHub API operations at a fixed bound, accepted a retried delete's 404 as already converged, and failed permanent errors immediately. A real invocation against the simulator fixture package encountered two HTTP 502 responses before succeeding on its third attempt under macOS Bash 3.2.

- **The moving dependency gate found Monaco Editor `0.56.0`, whose export map broke both consumers' legacy worker imports — fixed 2026-07-21.** Both package manifests and the lock advanced together, including Monaco's DOMPurify transitive update. The first-party editor and demo changed their editor, JSON, and TypeScript worker imports from the removed `monaco-editor/esm/vs/...` subpaths to Monaco's canonical exported subpaths. Both production bundles emitted the real worker assets, their 77 combined unit tests passed, and the complete Node and Terraform dependency-freshness gate returned green.

- **The complete container-mode and live portal suites could not start every production web process after immutable release validation became mandatory — fixed 2026-07-21.** The reusable end-to-end server harness and live ECS Playwright server inherited neither `APPLICATION_RELEASE_REVISION` nor the image-baked `EDD_BUILD_SHA`, so real cloud/browser files failed at process startup before exercising their workflows. Both Playwright production-server launchers shared one release-environment resolver, and the package harness followed the same contract: each derived the exact checked-out `git rev-parse HEAD` only when the caller supplied neither revision. Explicit application or image revisions remained authoritative. Real checkout-backed tests covered all three resolution cases and asserted both Playwright web-server configurations carried the resulting immutable provenance.

- **The simulator fixture built and published successfully but CI failed during package retention — fixed 2026-07-21.** The workflow combined `gh api --paginate --slurp` with `--jq`, which the current GitHub CLI rejects. Retention now streams each page as creation-time/id rows, sorts the complete stream, and deletes only rows after the newest 20. The publication contract rejects the incompatible option pair, and a read-only invocation against the real fixture package verified the corrected pagination pipeline.

- **Relying-party logout depended on a direct app landing that could not carry Shauth's one-time completion correlation, and CI allowed multi-hour jobs — fixed 2026-07-21.** ECS Dev Desktop registered an app-owned `/auth/shauth/logout/complete` bridge that ignored every caller-controlled destination, redirected only to Shauth's issuer-origin completion endpoint, and returned through Shauth's correlation to the stable local `/signed-out` page. Real Chromium covered direct and catalog login, silent SSO, validation identity and release, local and provider logout, Back-Channel Logout revocation, hostile bridge inputs, replay, and local recovery against merged Shauth commit `08f5a78fb8b159fcbfe8317f24f430dbdfd3ed56`. Every GitHub Actions job received a literal timeout of at most 15 minutes, and previously long end-to-end, Terraform-simulator, and multi-architecture image work was split into bounded jobs.

- **Shauth validator credentials could cross the application trust boundary, local-login rejection surfaced HTTP 500, and terminal acceptance did not prove PTY teardown — fixed 2026-07-20.** The production-browser contract generated a sentinel validator password outside the ECS Dev Desktop runtime and proved that Basic auth, bearer/API-key auth, development identity headers and cookies, and the local-account form all rejected it. Shauth's real bootstrap administrator password was injected only into the Shauth Compose processes and the isolated browser validator instead of being inherited by ECS Dev Desktop; Terraform rejected Shauth bootstrap and validator variables from both task-environment inputs. A real exact-issuer Shauth OpenID Connect exchange remained the only successful path. The app-owned `/auth/validation` page failed closed for anonymous callers and reported the exact signed-in username, email, normalized role, ordinary Sign out control, and immutable deployment-neutral release revision. Invalid local credentials returned to a stable login error instead of throwing through the server action. The signed-out landing remained on the ECS Dev Desktop origin with exact `Sign in with Shauth` text in accessible light and dark themes. The first-party terminal sent an explicit close signal, terminated the PTY process group, and verified two real shells, one-shell teardown, and stop/wake persistence through browser input. The general end-to-end job installed its pinned Chromium runtime before the terminal browser lifecycle and guarded cleanup of a browser that failed to start. Podman live tests used its native host-container coordinate instead of the simulator's unreachable rewritten outer gateway, restoring real idle heartbeats and SSH authorization. Required language-image and GitHub App suites also failed on missing dependencies instead of reporting skipped success.
...
- **Shauth catalog entry crashed, its confidential-client exchange used the wrong authentication method, relying-party logout immediately signed the user back in, and the provider-session index broke every non-Shauth callback — fixed 2026-07-20.** The catalog route called Auth.js from a React Server Component, where callback/state/PKCE/nonce cookie mutation was forbidden, and the OpenID Connect provider inherited Auth.js's `client_secret_basic` default even though the Hydra client required `client_secret_post`. Application logout also cleared only the local Auth.js cookie before returning to a still-active Shauth browser session. The launch became a cookie-capable Next.js Route Handler and the provider explicitly selected `client_secret_post`. Shauth sign-in persisted the verified provider `sid`, `sub`, and ID token in the durable session ledger; application sign-out used Shauth's standard RP-Initiated Logout endpoint; and a signed OIDC Back-Channel Logout endpoint revoked all local sessions correlated through `sid`, `sub`, or both. Transactionally written primary-index correlation records removed the DynamoDB global-secondary-index consistency race, and TTL-backed consumed-`jti` records rejected replayed logout tokens. The post-logout redirect became the registered EDD-origin `/signed-out` page instead of the invalid Shauth portal origin, while non-Shauth sign-out returned to the ordinary login page and did not claim that a global Shauth session had ended. Every durable session also received complete provider-session facets, so GitHub, Microsoft Entra ID, and local-account callbacks no longer failed ElectroDB validation. Invalid issuer, audience, age, expiry, event, correlation, `jti`, signature, and nonce shapes failed closed; oversized unbounded request bodies were streamed only to the declared limit.

- **The new-session catalog could remain empty until a hard refresh, and its production-browser harness emitted avoidable AWS API errors — fixed 2026-07-20.** A process-level ten-second catalog cache could be populated before an administrator created the first image, while `/sessions/new` rendered that one stale snapshot forever. The launcher polled the authenticated catalog API, preserved a still-valid selection, selected the first newly available image, and surfaced refresh failures instead of requiring a reload. The production Chromium setup also supplied the same AWS region/endpoint coordinates as the application and created the three real Amazon ECR repositories through the Sockerless AWS simulator, eliminating false `AWS_REGION` and `RepositoryNotFoundException` server errors.

- **First publication failed by treating absent deployment resources and a disabled SSH gateway as release defects — FIXED (2026-07-19).** The release job coupled multi-architecture manifest publication to an unconditional roll of the control plane, reconciler, and SSH gateway. A fresh environment therefore failed before Terraform could consume its first images, and an established deployment with intentionally disabled SSH still failed on the absent gateway task definition. It also derived `<name>-workspaces` instead of accepting the shared Amazon ECS cluster coordinate. Release bootstrap now uses the configured cluster, discovers the actual Amazon ECS and Amazon EventBridge Scheduler topology, publishes explicit deployment and SSH booleans, and scopes its role to the enabled topology. Publication always completes, deployment runs only after the required resources exist, and the release artifact tells the post-deployment workflow whether a roll actually occurred. Sockerless AWS simulator coverage exercised both enabled and disabled SSH topologies through the same AWS CLI path used in production.
