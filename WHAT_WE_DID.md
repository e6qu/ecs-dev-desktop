...
- **2026-06-07** — **CI failures on PR #54 diagnosed and fixed (3 issues).** Three CI failures diagnosed and fixed before merging to `main`:
  ...
  - **e2e/e2e-https (Teleport auth exits 1):** `edd-teleport-auth` crashed immediately (0.5 s) after start. `docker-compose.ssh.yml` lacked a pre-creation step for the `edd-e2e-sessions` S3 bucket. Teleport 18.x validates S3 backend connectivity on startup (including `GetBucketLocation` / `HeadBucket`); the sim returns `NoSuchBucket` 404, causing Teleport to exit 1. Fixed by adding a `bucket-init` init container (`amazon/aws-cli`) that creates the bucket after `sockerless-aws-ssh` is healthy, before `teleport-auth` starts. Also added `docker logs edd-teleport-auth` capture on each failed harness retry so future failures are self-documenting.
