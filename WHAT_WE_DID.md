...
- **2026-07-03 — sockerless #761 fixes the integrated CloudWatch alarm race; submodule re-pinned to `354c81d3`.** After #759 added only a regression test, **e6qu/sockerless#760** was filed. sockerless **#761** fixed #760 (and #758) by moving the CloudWatch alarm evaluator's state read, action dispatch, and state write into a single `cwAlarms.Update` callback, preventing a concurrent `PutMetricAlarm` replacement from racing the in-flight tick. It also added Info-level logging for transitions, dispatch attempts, and SNS→SQS delivery skips. Re-pinned `third_party/sockerless` to `354c81d3`. PR #180 is now running through CI for verification.
