...
**2026-09-07 — Git access became configuration-honest, and users got platform-held GitHub SSH keys.** The dev environment could not clone private repositories because it configured neither a GitHub App nor GitHub sign-in, while its launcher offered a dead "Connect GitHub" link. The deployment's git-access paths (App / account linking / user SSH keys / none) were derived purely from configuration, surfaced on the Health board with a live App check, reflected in the launcher, and documented. Per the user's decisions (private halves encrypted in DynamoDB and delivered into tmpfs at boot; per-user, named keys), a `GitSshKeyService` generated ed25519 keys through an injected ssh2 port, the workspace image installed them at every boot with pinned host keys, and the launcher accepted SSH clone URLs whose create-time check ran `git-upload-pack` over ssh2 with each key. A stale 10 s catalog cache that hid a just-added base image from the launcher gained write-side invalidation.

**2026-09-07 — Session creation checked the repository first; the light theme passed a whole-portal accessibility audit.** A workspace created from a mistyped public repository URL had booted and then failed its clone with git's opaque prompt error and a misleading broker `404`, because git hosts answer "authentication required" for a missing repository exactly as for a private one. The create request now performed the smart-HTTP ref advertisement `git clone` itself starts with, using the owner's credential when one existed, and refused with a `422` that named the URL and the fix, a missing branch or tag, or an unreachable host; the broker answered `204` for an owner with no linked account; the launcher accepted any HTTPS git host. The light theme's dark top bar and 3.3:1 accent were traced to a media-query override losing to a later equal-specificity rule and to an accent used as text; both became tokens, and an axe-core sweep of every route in both colour schemes became a gate. The editor proxy absorbed OpenVSCode's always-missing `vsda` asset fetches. Simulator conformance gaps found in the dev environment's boot timeline were filed upstream (`#931`); the simulator pin's next bump became an open decision because upstream had moved the simulators into `sockerless-cloud`.

**2026-07-17 — ECS Dev Desktop composed into an existing shared environment.** The Terraform module accepted an environment-owned Amazon Virtual Private Cloud (VPC), public/private subnet coordinates, and Amazon Elastic Container Service (ECS) cluster ARN/name through explicit switches with complete precondition validation. It continued to create the platform's own security groups, IAM, data, registry, load-balancer, scheduler, and service resources while leaving the shared VPC, NAT, VPC endpoints, and ECS cluster unmodified. The live Sockerless Amazon Web Services simulator fixture instantiated both standalone and shared consumers, asserted that the shared control-plane service used the environment cluster and that no duplicate network/cluster resources entered its state, and asserted each consumer's Application Auto Scaling target by exact service identifier. The DNS/TLS fidelity probe selected the standalone target by the same identifier rather than relying on a global count. The fixture verified a no-change plan and destroyed the complete stack. The same change refreshed AWS SDK clients to their current age-eligible releases, as well as Vite and Knip; TypeScript remained at 6.0.3 because the latest TypeScript ESLint peer range did not support TypeScript 7.
...
**2026-07-12 — Terminal tabs gained rename + reorder; the close→no-stale-session guarantee was proven; opencode blank render root-caused (base-path routing).** Verified the deployed app end-to-end (my own live browser drive of all four editors on `4b511a9`, plus dissecting the post-deploy smoke's assertions): workspaces launch and the **terminal** editor fully works — a tab opens by default, typed commands run and write files (verified via the container file API), and new-tab/switch/close all work. Then extended the first-party terminal SPA (`services/editor-monaco`): each tab is now **renameable** (double-click → inline input; Enter/blur commits, Escape cancels) and **nameable/no-name** (an empty name reverts to the default, which is now the stable id-based `Terminal <id>` — fixing the old `tabs.length+1` scheme that produced duplicate labels after close-then-open), and tabs are **reorderable** by drag-and-drop AND by an accessible `Alt+Shift+←/→` keyboard move. The tricky ordering/labeling (default label, name normalization, `moveInArray`, next-active-after-close) was extracted to a pure, unit-tested `spa/tab-order.ts`. Proved the **no-stale-session-on-close** guarantee: made the PTY spawner injectable (`terminal.ts` `PtySpawner`/`PtyLike`, threaded through `createEditorServer`) and unit-tested that a socket close, a PTY exit, and a raced close (socket gone before spawn resolves) each `kill()` the PTY. All terminal behaviors were verified in a real Chromium against the built SPA with an in-process fake PTY (11/11 checks: default tab, type→echo, new tab, id-based labels, switch, rename, clear-to-default, keyboard reorder, drag reorder, close→1 tab, close→PTY reaped); the deployed smoke's `assertTerminalWorkflow` was extended to cover rename + reorder on the real image. **opencode** was root-caused (live probes of a fresh workspace): the proxy delivers everything correctly (bundle loads, all 8 bootstrap APIs 200 incl. streamed SSE, a real provider present, zero errors) but the SolidJS SPA mounts and paints only its out-of-`<Routes>` `dialog-stack` — its client-side **path router matches no route under the `/w/<id>/` proxy prefix**, so the main view never renders. The earlier "import-map residual risk" note is disproven (single-file bundle, no `/assets/*` 404; SSE delivery also confirmed via a live EventSource). The clean fix — base-path virtualization (strip `/w/<id>` from `location.pathname`/`href` reads so the router matches as `/`, re-add on history writes) — was ATTEMPTED live (Playwright `addInitScript`, pre-bundle) and found **technically impossible**: Location's path accessors are [Unforgeable] Web-IDL properties (non-configurable, `location` non-reassignable), so the getter can't be overridden. `opencode-ai@1.17.15` has no base-path option (open upstream request; a third-party prefix-aware fork exists). So opencode is BLOCKED with no clean in-repo fix; recorded in `BUGS.md` + `DO_NEXT.md` with the options (track upstream / vendor the fork / drop opencode / per-workspace subdomain) for a decision. Full build + lint + unit tests green.

**2026-07-12 (later) — opencode blank render FIXED via a targeted proxy base-path patch.** The previous entry recorded opencode as blocked (base-path routing; inline-shim virtualization impossible because `location.pathname` is [Unforgeable]). Re-attacked it by patching opencode itself. Pulled the exact served bundle from a live workspace and found the router is a Solid-Router-style path integration whose `get` reads `window.location.pathname.replace(/^\/+/,"/")` — a UNIQUE, minified anchor (its `set` counterpart `window.history.pushState(o,"",r)` is also unique). The proxy now buffers opencode's JS bundle and applies ONE exact-string replacement redirecting that read through `window.__eddStrip` (`patchOpencodeRouterBase` in `workspace-proxy.ts`), so the router matches as if at `/`; the injected shim gained `__eddStrip` (strip the `/w/<id>` base) and wraps `history.pushState/replaceState` to re-add the base on WRITES so the real URL stays under the workspace prefix (reload/bookmark keep working). This is a TARGETED, verified-unique, FAIL-LOUD edit (throws → 502 if the anchor is ever missing after an opencode bump) — categorically different from the blanket JS regex that once corrupted the bundle. Verified live: intercepting the read (Playwright route) took `#root` from the blank `dialog-stack` to the real header + main layout with the URL unchanged; the exact edit applied to the real 2.75 MB bundle yields valid JS (`node --check`) with exactly one replacement. Decided (user) to keep this proxy patch over building opencode from source — opencode's web UI is embedded in a 135 MB Bun binary (no npm source / on-disk asset / external-web-dir), so a "source patch" would mean owning an opencode build. Unit tests cover the shim (`__eddStrip` + history wrap) and `patchOpencodeRouterBase` (patches the anchor, leaves non-router JS untouched, fails loud on drift); the smoke's `assertOpencodeMounted` was strengthened to require the routed UI (header/control), not just a mounted `#root`. Full build + lint + unit tests green.

**2026-07-12 (later) — Workspace pwd cleaned of editor/software leakage + thorough usefulness verification.** First proved workspaces are genuinely useful: drove a real deployed terminal workspace through complex real dev work (results captured via the confined file API) — a multi-file Node ESM project (`hi edd`), a compiled+run **Go** binary (`go-works 42`), **Python** (`py-works 45`), **git** init/commit/log, an **npm install from the network** + run (`is-odd: true`, so egress + registry work), and a **make** build — all succeeded; toolchain present (bash/node/npm/python3/pip3/go/git/cc/make/curl). That same probe surfaced the leak the user flagged: on a FRESH terminal workspace `ls -A $HOME` showed `.config` + `.openvscode-server`, i.e. the pwd was not empty. Root cause (filesystem map): the EBS volume was mounted at `/home/workspace`, which was simultaneously `$HOME`, the shell cwd, AND the editor's opened folder, so every editor/CLI dotfile landed in the user-visible pwd. **Fix (user-chosen topology):** the persisted volume now mounts at `/data`, split so editor state never pollutes the project — `/data/project` is the pwd / shell cwd / editor opened folder (clean, empty for a fresh workspace; a cloned repo lands in a subdir), `$HOME` = `/data/home` (editor/tool config, state, caches, shell history — persisted but OUT of the project), and `/data/extensions` holds the writable, persisted OpenVSCode user extensions (out of the project). Editor software stays read-only under `/opt` + `/usr/local` (baked). Implemented across `@edd/config` (new `DEFAULT_WORKSPACE_{MOUNT,PROJECT,HOME,EXTENSIONS}_PATH`), `editor-monaco/run.ts` (root → project), base + variant Dockerfiles (HOME/`NPM_CONFIG_PREFIX`/`GOPATH`/PATH/`useradd -d`/`WORKDIR`/sshd + profile.d, incl. a profile.d drop-in so interactive SSH logins also start in `/data/project`), and `entrypoint.sh` (dir layout, openvscode `--extensions-dir`/`--user-data-dir`/`--default-folder`, repo clone into the project, and an IDEMPOTENT first-party-extension copy so it is a one-time first-boot step, not per-start work — honoring "baked at image build, no build at start"). Updated the data-durability/image-variants/workspace-toolchain e2e assertions to the new paths, and added a `assertProjectDirClean` gate to the deployed smoke (fresh terminal + monaco workspaces must have no editor-state dotfiles in the project tree). Local build + lint + affected unit tests green (config 13, editor-monaco 37, compute-ecs 39, proxy 64) + shellcheck; full end-to-end validated by the container-mode e2e (builds the real image) + post-deploy smoke. Then eliminated the LAST runtime step: `edd-workspace-ui` is now packaged to a `.vsix` (`@vscode/vsce@3.9.2`) and `--install-extension`ed into the built-in extensions dir at IMAGE BUILD, so it registers as a real built-in — read-only (root-owned) and non-user-modifiable, loading with NO first-boot copy (verified in a linux container via `--list-extensions`). So a fresh workspace is now genuinely fully-built at start (mkdir of the persisted layout is the only first-attach action). The user's own extensions still install into `/data/extensions`.

**2026-07-13 — Added S3 + DynamoDB gateway VPC endpoints (cut NAT data-processing cost).** Added two gateway VPC endpoints (`infra/terraform/modules/ecs-dev-desktop/vpc_endpoints.tf`) — `com.amazonaws.<region>.s3` and `...dynamodb` — associated with the private route tables (`aws_route_table.private[*]`). Gateway endpoints are FREE (no hourly or per-GB charge, unlike interface endpoints) and route that traffic to the service over the AWS backbone instead of out through the fck-nat instance. Wins: every DynamoDB call (the single-table control-plane store, read on each request/heartbeat) and all S3 traffic now bypass NAT — the biggest saving being ECR image-layer pulls, since ECR stores layers in S3 and the multi-GB golden-image cold-start pulls stop paying NAT data-processing. Purely additive + transparent: AWS injects the managed-prefix-list route automatically, the SDK keeps using the regional endpoints (no app change), and no security-group change is needed (a gateway endpoint is a routing target, not an ENI). Default full-access endpoint policy (IAM task roles + SGs still govern access); exported the two endpoint ids as outputs. `terraform fmt`/`validate` clean, Trivy IaC clean; no provider change so no lock regeneration.

**2026-07-13 — Terraform: ignore release-pipeline-owned task-definitions so `apply` never reverts the running image.** Prepping to `terraform apply` the new S3/DynamoDB gateway endpoints (#233) surfaced a latent hazard: the control-plane + ssh-gateway ECS **services** and the reconciler **Scheduler schedule** all had their `task_definition` MANAGED by Terraform, but the RELEASE pipeline (`scripts/deploy-release-images.sh`) rolls them to fresh task-def revisions out-of-band on every deploy. So ANY later `terraform apply` — even an infra-only one like adding VPC endpoints — would have reverted those three to the Terraform-managed (stale) revision, rolling the live app back to an old image. Fixed in code by adding `lifecycle { ignore_changes = [...] }` on the release-managed task-def attribute of each: `aws_ecs_service.control_plane` (added `task_definition` alongside the existing `desired_count`), `aws_ecs_service.ssh_gateway` (new lifecycle block), and `aws_scheduler_schedule.reconciler` (`target[0].ecs_parameters[0].task_definition_arn`). Terraform still creates the initial task-defs + services/schedule; the pipeline owns image rolls thereafter — so applies are now idempotent w.r.t. deploys. `terraform fmt` + `validate` clean. (Production coordinates were already safe: `install.tfvars` is gitignored + untracked; the tracked `terraform.tfvars.example` uses placeholders only; the S3 state backend is an empty `backend "s3" {}` configured at `init` via `-backend-config`, never in code.) The gateway endpoints can now be applied to real AWS safely once this merges.

**2026-07-13 (later) — opencode terminal overlay + clean project dir; terminal fixes; ARM64/Graviton multiarch; startup hardening.** A chunky sweep, verified live on a full arm64 prod cutover. **opencode now opens the clean `/data/project`**: root-caused (opencode 1.17.15 web opens the git WORKTREE of its cwd, so a non-git `/data/project` walked up to `/` — confirmed via opencode's `/path` endpoint) and fixed by `git init`-ing the project dir on first boot in the entrypoint opencode branch (idempotent); verified live the "Recent projects" now shows `/data/project`. **opencode gained a terminal it does not ship**: the first-party terminal server runs as a SIDECAR on `:3001` under `/w/<id>/__edd_term/` (new `@edd/config` `DEFAULT_WORKSPACE_TERMINAL_PORT` + `WORKSPACE_TERMINAL_OVERLAY_SEGMENT`; `editor-monaco/run.ts` gained an `EDD_BASE_PATH` override), and the control-plane proxy injects a persistent bottom-left "⌨ Terminal" toggle + an on-top, minimizable overlay iframe into opencode's HTML, routes the sub-path (HTTP + PTY WS) to the sidecar, and widens opencode's CSP with `frame-src 'self'` (toggle logic rides the already-whitelisted shim hash). Tokenless — reachable only via the session-authorizing proxy + the workspace SG (new terraform ingress rule opens `:3001` workspaces←control-plane). The injected toggle button was verified live; end-to-end click-through is gated behind the `:3001` SG rule. **Terminal (editor-monaco)**: plain tabs now run a LOGIN shell (`-l -i`) so tools get the full `/etc/profile` PATH (fixes "can't run the opencode CLI" in a terminal; verified `opencode --version` in a login-shell tab on the image); pre-open keystrokes are buffered (first-command-drop fix); a window resize no longer un-maximizes; the SPA home link hides when embedded in the overlay; the esbuild server bundle is **minified** (688→384 KB). **Multiarch (ARM64 default)**: workspaces + services default to ARM64/Graviton — `runtimePlatform` in `compute-ecs` and the three service task defs (terraform), and `deploy-release-images.sh` now FORCES the arch (the services `ignore_changes[task_definition]`, so nothing else flips them). `publish-images.sh` gained `EDD_SKIP_MANIFEST`/`EDD_MANIFEST_ONLY`; `release.yml` + `golden-images.yml` build per-arch on NATIVE runners (arm64 on `ubuntu-24.04-arm`) then a manifest job — no QEMU. The `repo:tag` + `repo:tag-arm64` + `repo:tag-amd64` convention was documented in `AGENTS.md §4`. **Startup**: SSH host keys are now PERSISTED on the volume (stable across scale-to-zero wakes — no more host-key-changed warnings — and skips keygen on wake); the settings-seed `node` spawn is skipped when settings already exist. **Deploy + verification**: migrated to a full arm64 cutover — pushed arm64 `control-plane`/`ssh-gateway`/`omnibus` (each a `:<tag>` manifest + `-arm64`), rolled all three services to ARM64 (control-plane task confirmed running `arm64`, healthz + login 200 through the zero-downtime rollout, circuit-breaker armed), and drove the deployed Playwright smoke on the arm64 image: **monaco** (edit/save), **openvscode** (real File menu), **terminal** (default tab, run+write, new/switch/rename/reorder/close, clean pwd, opencode CLI present), and **opencode** (routed UI + injected terminal toggle + `/data/project` worktree) all passed, cleaned up with zero leaked workspaces. The smoke harness gained an `EDD_VERIFY_BASE_IMAGE` override + idempotent catalog registration, an `EDD_VERIFY_EDITORS` subset filter, an opencode cwd assertion, and an opencode terminal-overlay assertion (gated by `EDD_VERIFY_OVERLAY`). Full pre-commit gate green (eslint, tsc, unit tests incl. 286 proxy + 41 compute-ecs + 37 editor-monaco, lockfile, knip, jscpd) + shellcheck + actionlint. Also migrated prod Terraform state local→S3 (see the `edd-prod-terraform-state` memory) and applied the #233 gateway endpoints.

**2026-07-13 (later still) — Control-plane scale-to-zero WAKE path fixed: Lambda Function URLs are dead in this account, so the wake Lambda moved behind an API Gateway HTTP API, gated by a CloudFront shared-secret header.** After #239 redesigned the wake to a single ALB origin + a 503 `custom_error_response` → wake Lambda, the wake Lambda's front door refused every request. Diagnosed on prod: the Lambda **Function URL returned `403 AccessDeniedException` at the URL auth layer with ZERO invocations** under BOTH tried configs — (a) `AuthType AWS_IAM` + a CloudFront lambda-type **OAC** (sigv4) + a distribution-scoped `InvokeFunctionUrl` grant, and (b) `AuthType NONE` (public) + a valid public resource policy (`Principal:"*"`, condition `FunctionUrlAuthType=NONE`). All config verified correct; propagation ruled out (>5 min; a freshly deleted+recreated NONE URL failed identically). **Decisive evidence:** a direct SDK `aws lambda invoke` of the same function returned **200** with the reload page and the token honored — so the function works and invocation is not blocked account-wide; only the **Function-URL front door** is (this account is not in an AWS Org, so not an SCP — a Function-URL-specific account restriction). **Fix:** front the same wake Lambda with an **API Gateway HTTP API** (`aws_apigatewayv2_api` + `AWS_PROXY` integration [payload format **2.0**, byte-identical to the Function-URL event shape → handler unchanged] + `$default` route + `$default` auto-deploy stage + an `apigateway.amazonaws.com` invoke permission), which uses the STANDARD invoke path (the one that works) and is a first-class CloudFront origin. Access control is unchanged and coordinate-clean: a **48-char `random_password.wake_token`** is injected by CloudFront as the `x-edd-wake-token` custom origin header and verified by the handler (new `WAKE_TOKEN_HEADER` gate in `packages/wake-listener` — returns 403 before any ECS call on mismatch; 4 new unit tests), so the public API is reachable only through CloudFront (and even a bypass only ever triggers an idempotent `ecs:UpdateService`). The `random` provider was added to the module `versions.tf` and the provider lock regenerated across all platforms. **Applied to prod (targeted, twice user-approved — the public front door + the API-Gateway shape are both security-relevant, classifier-gated changes the user named) and verified live:** API direct → 403 without the token, 200 reload page with it; and **`https://app.edd.e6qu.dev/_edd_wake` through CloudFront → 200 reload page** (CloudFront header-injection → API Gateway → token gate → handler confirmed end-to-end). Two unrelated pre-existing prod drifts were found by the plan and deliberately EXCLUDED from the targeted applies (recorded in `DO_NEXT.md`): the fck-nat instance wants a launch-template-version REPLACEMENT (would drop workspace egress — investigate before applying), and a benign `+ManageCloudFrontWaf`/`+IntrospectCloudFrontWaf` control-plane IAM addition. **The live scale-from-zero loop was then VERIFIED end-to-end on prod (user-approved deliberate downtime):** scaled `edd-prod-control-plane` to 0 → at desired0/running0 `app.edd.e6qu.dev/` served the 200 reload page (CloudFront 503→wake) → the wake Lambda logged `control-plane wake from:0 to:2` (the real `ecs:UpdateService`) → desired bounced to 2 → tasks started → app recovered (`/`→307, `/login`→200), with subsequent requests logging idempotent `hold`; one transient ~1-cycle 502 during target warm-up is normal cold-start (only 503 = scaled-to-zero is routed to the wake page — 502/504 stay ambiguous). Shipped as **PR #240** (`fix/wake-scale-to-zero-apigw`). Still open (tracked): the two excluded prod drifts, and refreshing the stale CLI sim-conformance slice `tests/sim/adversarial-slice-cloudfront-wake-waf.sh` (still describes the dropped origin-group + Function-URL shapes; add apigatewayv2 coverage or SKIP-if-unsupported). Wake-listener build + lint + 12 unit tests green; `terraform validate`/`fmt` clean.

**2026-07-13 (follow-ups to the wake fix) — sim-conformance slice refreshed to the API-Gateway design; fck-nat replacement drift fully diagnosed.** Two decision-free follow-ups after PR #240 merged. **(1) Sim slice refresh:** `tests/sim/adversarial-slice-cloudfront-wake-waf.sh` still probed the DROPPED design (CloudFront origin group + failover on 502/503/504, a Lambda **Function URL** as the failover origin, a lambda-type OAC + a scoped `InvokeFunctionUrl` grant, and a reserved-concurrency ceiling). Rewrote it to probe the CURRENT shapes `cloudfront.tf` creates: a wake Lambda fronted by an **API Gateway HTTP API** (AWS_PROXY integration on **payload format 2.0** + `$default` route + `$default` auto-deploy stage + an `apigateway.amazonaws.com` `InvokeFunction` grant scoped to the API), a CloudFront distribution with a **SINGLE ALB origin** (asserts `OriginGroups.Quantity == 0`), a `/_edd_wake*` ordered behaviour targeting the API Gateway wake origin, a **503 `custom_error_response`** → `/_edd_wake` (response_code 200), and the wake origin carrying the **`x-edd-wake-token`** custom origin header (the shared-secret gate); the CLOUDFRONT-scope WAFv2 web ACL + IP set + per-IP rate-based BLOCK rule + WebACLId attach are unchanged. Verified: brought the sockerless sim up locally (base compose — the `docker-compose.tier2.host.yml` host-networking override doesn't publish ports on macOS) and ran the slice green end-to-end, twice, with clean teardown (0 leftover resources); shellcheck clean (used `"\$default"` double-quoted literals to avoid SC2016 on the API Gateway `$default` names). The sim implements every probed shape correctly (`apigatewayv2_complete.go`; CloudFront custom_header + custom_error_response round-trip) — **no simulator incongruity to file upstream**. **(2) fck-nat drift diagnosis:** traced why a non-targeted prod apply wants to REPLACE the fck-nat NAT instance (a workspace-egress hazard, kept out of every targeted apply this session). Root cause: the instance's launch-template version is `"$Latest"` (RaJiska/fck-nat 1.6.0, `auto_rollout` defaults false) but state pins concrete `"2"` (a force-new attribute → replacement); and fck-nat's `user_data` is **non-deterministic gzip'd cloudinit**, so a new (byte-identical) LT version is minted on essentially every apply — the live LT `edd-prod-nat` has v1/v2/**v3**, and v2≡v3 (same AMI/type/SG). So a one-time replace won't stick — the churn returns. Recorded the full diagnosis + decision options (pin `ami_id` + vendor/wrap fck-nat with `ignore_changes=[launch_template[0].version, user_data]`; one-time maintenance-window replace + keep targeting; or `nat_mode="gateway"`) in `BUGS.md` (Open, top) + `DO_NEXT.md`; until decided, keep prod applies targeted and fck-nat-excluded.

**2026-07-13 (later) — fck-nat NAT-instance replacement drift RESOLVED with `auto_rollout = true` (auto-update on apply).** The remaining prod hazard from the wake work — a non-targeted `terraform apply` wanting to REPLACE the fck-nat NAT instance (a workspace-egress blip) — was fixed by the user's chosen policy: let fck-nat auto-update on apply, pinned to a known OS. Corrected an earlier mis-diagnosis first: the claim that fck-nat's gzip cloudinit `user_data` was non-deterministic (churning a new LT version per apply) was DISPROVEN by evidence — launch-template v2 and v3 have byte-identical `user_data` (same md5) + same AMI/type/SG, so v3 was a one-off, not per-apply churn. The real drift was purely fck-nat's default `launch_template { version = "$Latest" }` (auto_rollout=false) vs the concrete `"2"` the AWS provider records in state — a force-new attribute, so any newer LT version left a standing `"2" → "$Latest"` replacement diff. Fix: set `auto_rollout = true` on the `fck_nat` module call (`nat_instance.tf`), so the version resolves to a CONCRETE `latest_version` tracked in state and the instance rolls ONLY when the launch template genuinely changes (a new fck-nat AMI). The OS family is already pinned by fck-nat's AMI filter to Amazon Linux 2023 (`fck-nat-al2023-hvm-*`, owner 568608671756, `most_recent = true`), so we track AL2023 and get its security updates without floating OS families. Verified by a read-only prod plan: `launch_template.version "2" → "3"` (concrete), LT resource unchanged (`0 to change`) → a single reconciliation roll, then clean plans until a new AMI ships. `terraform validate`/`fmt` clean; no provider change so no lock regeneration. The first prod apply carrying this rolls the NAT once (brief egress blip, accepted); afterwards full applies no longer need to `-target`-exclude fck-nat.

**2026-07-14 — Applied the fck-nat auto_rollout roll to prod + thorough system test surfaced (and fixed) a 14h reconciler outage.** Ran the user-approved targeted prod apply for the fck-nat change (auto_rollout roll: NAT instance `i-05e6…`→`i-018e…`) + the `+ManageCloudFrontWaf`/`+IntrospectCloudFrontWaf` control-plane IAM statements. Verified the roll against REALITY, not terraform state: the new NAT's static ENI re-attached with `SourceDestCheck=false`, both private route tables `active` (recovered from a brief mid-boot blackhole while fck-nat cloud-init re-attached the ENI), and — the definitive proof — a one-shot Fargate task in a PRIVATE subnet (no public IP) curled `checkip.amazonaws.com` and got `3.255.247.210` (the new NAT's public IP), exit 0, so real private-subnet egress works through the new NAT. Then ran the full deployed browser smoke: all four editors (openvscode, monaco, terminal, opencode) verified `IDE + workspace healthy` with screenshots. **The smoke's CLEANUP step failed, surfacing a pre-existing incident unrelated to the NAT roll:** 27 orphaned RUNNING workspace ECS tasks (deleted workspaces whose Fargate tasks were never reaped, spanning 6 image-tag families, earliest from the prior day) because **the reconciler had been down ~14h** (since 2026-07-13 11:33 UTC). Root cause (DLQ evidence, 176 messages): the `edd-prod-scheduler` role lacked `ecs:TagResource`, which the schedule target's tag propagation (`enable_ecs_managed_tags`/`propagate_tags`/`tags`) requires on the created task — so every RunTask tick failed `AccessDeniedException` into the DLQ (no reaping, no scale-to-zero, no snapshots). iam.tf already grants this to the control-plane role and documents the requirement; the scheduler role was missed. Fixed by adding a cluster-scoped `TagReconcilerTask` (`ecs:TagResource`) statement to `data.aws_iam_policy_document.scheduler`; applied to prod (additive IAM) and confirmed the reconciler resumes + reaps the orphans. Latent follow-up recorded: `listWorkspaceTasks` (compute-ecs) throws the whole reaper sweep on any single `DescribeTasks` failure (all-or-nothing) — harden to tolerate per-task MISSING.

**2026-07-14 — Moved the environment config to `github.com/e6qu/infra` (Terragrunt) and rebuilt the platform fresh as env `edd` (renamed from `edd-prod`).** The reusable module stays in `ecs-dev-desktop`; only the environment config moved, matching `e6qu/infra`'s convention ("Terragrunt environments only; modules remain with their owning services", alongside the existing `bleephub/` unit). Authored `e6qu/infra/edd/terragrunt.hcl` (PR e6qu/infra#1): module pinned by commit, S3 remote state (`edd-terraform-state-729079515331`, `use_lockfile`), generates both the regional + `us-east-1` aliased providers, all inputs (name=`edd`, domain `edd.e6qu.dev` unchanged, `nat_mode=instance`, `image_build_mode=codebuild`, secrets by ARN only). This was a **destructive rebuild** (user-approved, alpha, no real data): captured + preserved the external GitHub OAuth id/secret + image-source webhook secret, bootstrapped fresh `edd/*` state + secrets (crypto regenerated) + the `edd-github-release` release OIDC role, destroyed `edd-prod` (`uninstall.sh` + manual deletion-protection/force-delete clearing on the 2 LBs/DynamoDB/ECR since the module's destroy path needs those out-of-band), then `terragrunt apply`ed `edd`. The apply surfaced a chain of latent CodeBuild/publish bugs (see BUGS.md → Open, top): the `edd-base` ECR grant + `deletion_protection` exposure (fixed in PR #244), CodeBuild IAM-propagation lag, per-arch immutable-tag idempotency, and — the blocker — `image_build_mode=codebuild` being amd64-only while services/workspaces pin ARM64. Resolved the arm64 gap the intended way: dispatched the native-multiarch **release + golden pipelines** (`tag=5bc2911`), which built arm64+amd64 and rolled the services. **Verified end-to-end on prod:** `terragrunt plan` clean; `app.edd.e6qu.dev` healthz/readyz/login (ok/ready/200); services arm64 (control-plane 2/2, ssh-gateway 1/1); the deployed browser smoke passed all four editors + workspace lifecycle with clean cleanup; reconciler healthy (DLQ 0, RunTask SUCCESS — the #243 fix holds in the fresh stack); real egress through the new fck-nat (`34.244.146.100`); SSH NLB `:22` open. **No-stale sweep clean:** zero `edd-prod` resources across ECS/EC2/ELB/DynamoDB/ECR/Secrets/IAM/CloudWatch/SQS/Scheduler/Budgets/CloudFront/WAF/CodeBuild; deleted the old `edd-tfstate-edd-prod` state bucket, the ECS container-insights log group, and the stale `edd-prod-github-release` OIDC role (only the tiny idle `edd-tfstate-locks` table remains — classifier-gated delete).

# Dependency freshness

The dependency gate was restored by refreshing age-eligible workspace packages
...
**2026-07-19 — Hardened the complete Shauth lifecycle and made shared-environment upgrades state-safe.** The Shauth provider retained the existing refresh-token scope while requiring PKCE, state, and nonce and validating the exact UserInfo subject, username, email, picture, and developer/admin role. GitHub, Microsoft Entra ID, and Shauth were each registered and displayed only with complete confidential-client configuration; absent providers disappeared and partial credentials failed loudly instead of exposing dead login controls. Insecure, credential-bearing, query-bearing, and fragment-bearing Shauth coordinates also failed loudly. The new `/login/shauth` catalog coordinate entered Shauth directly. Logout revoked the durable Auth.js application session before clearing every chunked browser cookie and returned to the configured Shauth applications portal, preserving central SSO without leaving the application session fail-open. The shared-environment Terraform conversion gained explicit state moves for formerly singleton network, gateway-endpoint, and Amazon ECS cluster resources; a mock-provider plan proved that reuse mode created no duplicate network, NAT, endpoint, or cluster. The moving dependency-freshness gate was satisfied with AWS SDK `3.1090.0` across every client package and the shared lockfile. The complete pre-commit gate, 297 web tests, the 22-task production build, and the Terraform plan test passed after rebasing onto the merged Shauth and shared-environment work.

**2026-07-19 — Made every deployable ECS Dev Desktop image coordinate immutable and source-verifiable.** The control-plane Amazon ECR repository matched the already immutable golden and SSH-gateway repositories. The Terraform module and complete example required a 7-40 character lowercase hexadecimal Git commit prefix instead of silently selecting `main`; the installer and release deployer rejected missing or mutable tags; and a shared POSIX-shell validator kept those command paths identical. Image publication additionally rejected dirty source trees and tags that did not identify the checked-out commit, while retaining resumable per-architecture publication and the bare manifest plus `-amd64` and `-arm64` convention. Terragrunt, complete-example, simulator, and mock-provider fixtures used explicit immutable coordinates, and deployment documentation described the same contract. The Linux and macOS shell jobs followed shared-library source directives, the macOS job removed its unused untrusted Homebrew tap, and the simulator assertion required the immutable repository it actually provisioned. The API-only Terraform fixture kept its services at zero instead of requesting workloads from a runtime that deliberately had no container engine. ShellCheck, Terraform formatting and validation, the mock-provider module test, and negative mutable-tag tests passed. A real isolated Sockerless AWS simulator apply created 146 resources, verified immutable ECR plus desired/running `0/0`, replanned with no changes or missing-runtime errors, and destroyed all 146 resources.

**2026-07-19 — Separated first image publication from topology-aware deployment.** Release bootstrap accepted the configured Amazon ECS cluster instead of deriving `<name>-workspaces`, inspected the real task definitions and services plus the Amazon EventBridge Scheduler reconciler schedule, and published explicit deployment and optional SSH-gateway booleans. The release workflow continued to build native AMD64 and ARM64 images and their multi-architecture manifests before a complete environment existed, but rolled only components declared present. Release metadata always recorded whether deployment occurred, and the post-deployment workflow resolved that metadata before starting its expensive authenticated browser smoke. The release role excluded SSH service/task-definition permissions when SSH was disabled. The Terraform simulator contract exercised both disabled and enabled SSH release paths using the production AWS CLI code path. The complete pre-commit gate passed after the release scripts, workflows, documentation, and continuity state were synchronized.

**2026-07-19 — Made the Shauth catalog launch coordinate a real cookie-capable request.** The `/login/shauth` catalog route moved from a React Server Component to a Next.js Route Handler so Auth.js could write callback, state, PKCE, and nonce cookies before redirecting to the Shauth authorization endpoint. The production bundle returned a cookie-bearing 307 instead of the prior HTTP 500 Server Components failure, and regression coverage kept configured and unconfigured provider behavior explicit.

**2026-07-19 — Completed provider-coordinated Shauth logout without breaking other identity providers.** Shauth authorization persisted the already-verified provider session identifier, provider subject, and ID token beside the server-controlled application session. The sign-out action revoked the local Auth.js session and entered Shauth's standard RP-Initiated Logout flow with the exact registered EDD-origin `/signed-out` coordinate; configuration rejected the former cross-origin Shauth-portal URL that Ory Hydra could not accept. GitHub, Microsoft Entra ID, and local-account sign-out returned to the ordinary login page instead of falsely claiming that Shauth had ended a global session. The Back-Channel Logout receiver cryptographically verified Shauth's issuer, audience, signature, age, expiry, event, `jti`, standard `sid`/`sub` correlation, and absent `nonce`. It consumed each `jti` exactly once and revoked every matching durable application session through transactionally written, strongly-consistent primary-index pointers rather than a racy DynamoDB global secondary index. DynamoDB TTL expired replay and correlation records with the signed token and rolling application session lifetimes. Every non-Shauth session still received complete persistence facets, preventing the new ElectroDB access pattern from rejecting GitHub, Microsoft Entra ID, or local-account sign-ins. Real DynamoDB and asymmetric-JWT tests covered correlation, replay, and validation; direct route and server-action tests covered bounded receiver requests and both logout branches; the complete web unit and 156-test simulator integration suites passed; and Chromium rendered the hydrated signed-out landing.

**2026-07-20 — Closed the confidential-client and browser-level gaps in the Shauth repair.** The Shauth OpenID Connect provider explicitly used Hydra's registered `client_secret_post` token-endpoint authentication method instead of Auth.js's legacy default. Production Chromium coverage proved that the relying-party logout landing remained on ECS Dev Desktop, exposed local fresh-sign-in choices, and failed an unconfigured catalog launch closed on the local login page. The same run exposed a stale first-catalog-read race: `/sessions/new` polled the authenticated catalog, selected newly available images without a hard refresh, and surfaced refresh failures. Its simulator-backed browser setup created the required Amazon ECR repositories through the AWS API and supplied explicit AWS coordinates, so a green run no longer hid `AWS_REGION` or missing-repository server exceptions. The complete 314-test web unit suite, 156-test web integration suite, 29-test production Chromium suite, lint, and production build passed.

**2026-07-20 — Proved direct-entry ECS Dev Desktop SSO as one real browser lifecycle.** Unauthenticated `/` and `/workspaces` requests entered Shauth through a full-document navigation, preventing Next.js React Server Component fetching from turning the cross-origin OpenID Connect transition into a CORS failure. The Shauth catalog used `/` as the canonical launch coordinate and silently reused an existing Shauth browser session. ECS Dev Desktop logout ended the provider session and returned to the local `/signed-out` landing, while provider-initiated global logout delivered a signed Back-Channel Logout token that revoked the correlated durable ECS Dev Desktop session and made direct re-entry fail closed at Shauth. The production-browser contract also verified `/me`, browser-console cleanliness, and separate provider/relying-party cookie scopes. CI ran real Shauth and Ory Hydra services, a production Next.js bundle, and real DynamoDB and Amazon ECS resources provisioned through the pinned Sockerless AWS simulator; it introduced no mock identity provider, fake cloud implementation, synthetic response, or `oauth2-proxy` dependency. Full lint, unit/integration tests, production build, ShellCheck, and Chromium SSO coverage passed.

**2026-07-20 — Made Amazon ECR publication main-only, immutable, and structurally verifiable.** The release and golden-image workflows published only the checked-out `main` commit's 12-character prefix. Native AMD64 and ARM64 runners produced direct per-architecture OCI image manifests with provenance and SBOM indexes disabled; a separate manifest job assembled the unsuffixed AMD64+ARM64 OCI index. The control-plane, SSH gateway, shared `edd-base`, and every configured golden variant used the same manifest plus `-amd64` and `-arm64` convention. Publication verified those media types and platforms before deployment, rejected mutable, version, manual, and caller-selected tags, retained at most 20 Amazon ECR images per repository, and narrowed the GitHub Actions AWS OIDC role from main-plus-version-tags to the exact `main` ref. A shared pre-commit and CI contract kept the workflows, publisher, verifier, lifecycle, native ARM64 runner, and role trust synchronized.

**2026-07-20 — Made Terraform the sole deployment owner.** The release workflow stopped after publishing and verifying immutable OCI images; it no longer registered task definitions, updated Amazon ECS services, retargeted Amazon EventBridge Scheduler, emitted deployment metadata, or automatically launched post-deployment smoke. Its main-only AWS OIDC role was reduced to Amazon ECR push access and removed the legacy publish/deploy policy. A separate bootstrap created a main-only, least-privilege role for the explicitly dispatched real browser smoke, scoped to the exact Secrets Manager secret, DynamoDB table, and KMS key it used; the operator supplied the deployed secret ID and the bootstrap resolved its ARN instead of guessing a name. Terraform resumed ownership of the control-plane and optional SSH-gateway ECS service task-definition fields plus the reconciler Scheduler target. The simulator fixture accepted an immutable deployment tag, and saved-plan assertions required every task-definition replacement and corresponding attachment update. Complete default and DNS/TLS Sockerless AWS simulator rollouts applied a second immutable tag, proved through the real ECS and Scheduler APIs that the control-plane, reconciler, and SSH gateway used their newest revisions, replanned with zero changes, and destroyed all resources. The application retained one Application Load Balancer because its same-origin `/w/<id>/` editor and terminal transport required transparent raw WebSocket upgrade support; an API Gateway HTTP API could not preserve that contract, while splitting HTTP and WebSocket APIs would require separate custom-domain paths and additional ingress.

**2026-07-20 — Completed the Shauth trust boundary and real browser-terminal contract.** ECS Dev Desktop exposed immutable source revision and build time through its existing health endpoint and owned stable `/auth/validation` and `/signed-out` validation surfaces without knowing how Shauth was deployed. The authenticated validation page reported the exact username, email, normalized role, ordinary Sign out control, and deployment-neutral immutable release revision; anonymous access failed closed to the persistent app-local landing. A sentinel Shauth validator password existed only in the browser verifier process after the application started; Basic auth, bearer/API-key auth, development identity headers and cookies, and the local-account form all rejected it, while a real exact-issuer OpenID Connect exchange established the durable application session. Shauth's real bootstrap administrator password was injected only into Shauth and the isolated browser validator, never the ECS Dev Desktop process; Terraform rejected every Shauth bootstrap or validator variable from both the plain and secret control-plane environments. Invalid local credentials returned to a readable login error instead of surfacing a production HTTP 500. Relying-party logout ended the shared Shauth session and returned to ECS Dev Desktop's persistent signed-out page, whose exact `Sign in with Shauth` control met WCAG contrast in light and dark mode. The first-party Monaco terminal sent an explicit tab-close message, terminated the PTY process group, and verified two simultaneously typed real shells, single-tab process teardown, and storage persistence across stop and wake. The general end-to-end workflow installed its pinned Chromium runtime before running the terminal lifecycle, and teardown preserved the original startup failure if Chromium could not launch. The container-mode Sockerless harness selected Podman's native host-container coordinate instead of an unreachable rewritten outer gateway, restoring real idle heartbeats and SSH authorization. Required language-variant image and GitHub App suites moved from dependency-absent skips to explicit failing contracts. The real Shauth/Ory Hydra/PostgreSQL/Next.js/Chromium lifecycle passed, the 13-file live simulator suite passed 37/37 tests, and the portal browser suite passed 31/31 tests.

**2026-07-21 — Bound global logout to the application and capped every CI job.** ECS Dev Desktop replaced its direct post-logout landing registration with an app-owned `/auth/shauth/logout/complete` bridge. The bridge accepted no destination from the request, redirected only to Shauth's issuer-origin `/oauth/logout/complete`, and let Shauth consume its one-time correlation before returning to the persistent app-local `/signed-out` page. The authenticated `/auth/validation` surface reported exact username, email, role, and immutable release while its Sign out control exercised the same global flow. The real browser contract pinned merged Shauth commit `08f5a78fb8b159fcbfe8317f24f430dbdfd3ed56` and passed direct entry, catalog launch, silent SSO reuse, `/me`, validation, relying-party logout, provider logout, Back-Channel Logout revocation, hostile bridge parameters, completion replay, local recovery, and protected-API rejection against real PostgreSQL, Ory Hydra, the production Next.js build, and the Sockerless AWS simulator. Every GitHub Actions job had a literal timeout of at most 15 minutes; long end-to-end, Terraform-simulator, and golden-image work was split into bounded matrix and publication jobs while preserving exact required check names.

**2026-07-21 — Repaired GitHub Container Registry fixture retention against the current GitHub CLI.** The simulator fixture had built and pushed its real image before cleanup failed because `gh api` rejected the simultaneous `--slurp` and `--jq` options. The workflow streamed creation-time/id rows from every paginated response, sorted the combined stream newest-first, and retained exactly the first 20 before deleting older versions. The publication contract rejected the incompatible option pair, and the corrected read-only pipeline passed against the actual `edd-ci-sim-aws-exec` package.

**2026-07-21 — Kept immutable release provenance present in every real end-to-end web process.** The container-mode and live portal suites started the production custom server outside a published image, so neither deployment-owned `APPLICATION_RELEASE_REVISION` nor image-baked `EDD_BUILD_SHA` existed and immutable-release validation stopped their browser/cloud flows before they began. Both Playwright production-server launchers shared one release-environment resolver, while the package harness followed the same contract: each derived the exact checked-out Git revision and supplied it as `EDD_BUILD_SHA` only when no explicit revision existed. Real checkout-backed tests proved the derived value, proved both explicit revision forms remained unchanged, and asserted both Playwright configurations carried immutable provenance into their production web servers. The real Shauth lifecycle pin advanced to merged commit `08f5a78fb8b159fcbfe8317f24f430dbdfd3ed56` after its tree was verified identical to the reviewed predecessor.

**2026-07-21 — Split the real workspace fixtures, hardened GitHub Container Registry retention, and completed the Monaco Editor refresh.** A measured CI run showed the combined workspace fixture finished both image builds but exceeded its job ceiling during the serial upload. The shared base moved to an independently bounded, exact run-scoped package; the established workspace check consumed that coordinate through direct BuildKit publication; and the simulator published independently. The resulting base and workspace jobs completed in 5m17s and 12m12s. One POSIX-shell helper replaced three retention copies, kept exactly 20 versions per package, bounded retries to transient idempotent GitHub API failures, and converged against the real simulator fixture package after two HTTP 502 responses under macOS Bash 3.2. The moving freshness gate also advanced both Monaco Editor consumers to `0.56.0`; their worker imports followed the new canonical export map, both production bundles emitted the editor/JSON/TypeScript workers, 77 consumer tests passed, and the complete dependency gate returned green.
