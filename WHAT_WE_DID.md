...
**2026-07-19 — Completed provider-coordinated Shauth logout without breaking other identity providers.** Shauth authorization persisted the already-verified provider session identifier and ID token beside the server-controlled application session. The sign-out action revoked the local Auth.js session and entered Shauth's standard RP-Initiated Logout flow with the exact registered EDD-origin `/signed-out` coordinate; configuration rejected the former cross-origin Shauth-portal URL that Ory Hydra could not accept. GitHub, Microsoft Entra ID, and local-account sign-out returned to the ordinary login page instead of falsely claiming that Shauth had ended a global session. The Back-Channel Logout receiver cryptographically verified Shauth's issuer, audience, signature, age, event, `jti`, required `sid`, and absent `nonce`, then idempotently revoked every durable application session sharing that provider session. Every session received complete provider-session index facets, preventing the new ElectroDB access pattern from rejecting non-Shauth sign-ins. Real DynamoDB and asymmetric-JWT tests covered correlation and validation, direct server-action tests covered both logout branches, the complete web unit and integration suites passed, and Chromium rendered the hydrated signed-out landing.
