...
2. The private `e6qu/infra` development environment needed to pin that image; set `AUTH_URL=https://app.edd.dev.e6qu.dev` and `AUTH_SHAUTH_POST_LOGOUT_URL=https://app.edd.dev.e6qu.dev/signed-out`; register callback `https://app.edd.dev.e6qu.dev/api/auth/callback/shauth`, post-logout `https://app.edd.dev.e6qu.dev/signed-out`, and Back-Channel Logout `https://app.edd.dev.e6qu.dev/api/auth/shauth/backchannel-logout`; and apply it from exact synchronized `main`.
