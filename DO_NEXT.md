...
0. **Prod Terraform drift — mostly resolved 2026-07-13; CloudFront scale-to-zero REDESIGNED.**
   ...
   **Wake path — FIXED + live (2026-07-13); one live test remains.** Lambda Function URLs are
   ...
   `https://app.edd.e6qu.dev/_edd_wake` through CloudFront → 200 reload page. **Live scale-from-zero
   VERIFIED on prod (2026-07-13):** scaled CP→0 → served the wake page → wake Lambda logged
   `control-plane wake from:0 to:2` → desired bounced to 2 → app recovered (`/`→307, `/login`→200). The
   fix is in **PR #240** (`fix/wake-scale-to-zero-apigw`). Scale-to-zero + wake is DONE.
   Two leftover follow-ups from this work: (i) the two pre-existing prod drifts below (NAT replacement
   - WAF-IAM); (ii) refresh the stale sim slice `tests/sim/adversarial-slice-cloudfront-wake-waf.sh`
