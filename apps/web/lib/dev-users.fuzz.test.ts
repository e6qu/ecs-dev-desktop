...
// matchDevUser is the dev-auth credential gate; the exact-username + (password ?? fallback)
...
describe("matchDevUser (fuzz)", () => {
  it("matches iff exact username AND password === (user.password ?? fallback); never a different account", () => {
    fc.assert(
      fc.property(
        fc.array(userArb),
        fc.string(),
        fc.string(),
        fc.string(),
        (users, username, password, fallback) => {
          const result = matchDevUser(users, username, password, fallback);
          ...
          } else if (password === (first.password ?? fallback)) {
