...
// Default caps: viewer 0, member 5, admin unlimited (@edd/config DEFAULT_WORKSPACE_QUOTAS).
...
describe("getQuotaReport — over-limit flagging", () => {
  ...
  it("flags a member at/over their cap (5) and not below; carries role + limit", async () => {
    getFleetStatusMock.mockResolvedValue(
      fleet([
        { owner: "u-at", count: 5, role: "member" },
        { owner: "u-under", count: 3, role: "member" },
      ]),
    );
  ...
  it("unknown-role rows flag against the strictest POSITIVE cap (5), not the 0 viewer cap", async () => {
    ...
    const { usage } = await getQuotaReport();
    // No role known → limit stays null, but the flag uses the strictest positive cap (member=5).
