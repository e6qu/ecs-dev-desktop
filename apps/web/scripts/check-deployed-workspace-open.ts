...
type Editor = "openvscode" | "monaco" | "claude" | "codex";

const EDITORS: readonly Editor[] = ["openvscode", "monaco", "claude", "codex"];
...
async function authSecret(region: string, secretId: string): Promise<string> {
...
async function authJar(secret: string): Promise<{
...
async function firstEnabledImage(baseUrl: string, jar: readonly StoredCookie[]): Promise<string> {
...
async function waitReady(baseUrl: string, jar: readonly StoredCookie[], id: string): Promise<void> {
...
async function openEditor(
  ...
  const third = await fetchWithCookies(root, jar, { headers: docHeaders });
  if (third.status >= 400) {
...
const { jar, sessionId } = await authJar(secret);
