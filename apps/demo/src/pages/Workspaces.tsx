...
export function Workspaces(): JSX.Element {
  ...
  // A viewer is read-only (real CASL ability) — the create form + action buttons are hidden.
  const canMutate = cp.canMutateWorkspaces();
  ...
  return (
    <section className="demo-page">
      <div className="demo-page-head">
        ...
        {!canMutate && <span className="demo-readonly-note">viewer — read-only access</span>}
        {canMutate && (
          <form
      ...
      {mine.length === 0 ? (
        <p className="demo-empty">
          {canMutate
            ? "No workspaces yet — create one from a base image above."
            : "No workspaces to show for this identity."}
        </p>
      ) : (
