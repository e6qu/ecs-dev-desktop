...
| Auth.js callback routes                | `bleephub` + Azure/Entra sim                             | `apps/web/lib/nextauth-callback.e2e.ts`                           | The real NextAuth handlers: csrf → signin → IdP authorize → callback (code exchange, JWKS id_token checks) → session with the mapped role. Entra leg is TLS-only (`e2e-https`); Entra group→role interactively is gated on sockerless#547.                       |
...
| Entra group→role through the interactive Auth.js flow   | `nextauth-callback.e2e.ts` Entra leg, once sockerless#547 lands (`login_hint` user binding)                                         | The flow currently asserts the sim's default user/role; with #547 a Graph-provisioned grouped user becomes reachable interactively.     |
